...
If `.vehicle-embedded-docs/` exists, read it first and avoid repeating expensive full-document scans. Treat `portable/` as reusable hardware/software knowledge copied between projects, and `project/` as current-project schematics, code context, local links, features, and lessons. First-pass source indexes may be `metadata-only`; use them as navigation hints until targeted source pages are checked. On first use, generate candidate cross references from existing indexes before treating the memory as complete, including same-class software links among EB/DaVinci/tool guides, MCAL manuals, AUTOSAR standards, and vendor requirement extracts.
